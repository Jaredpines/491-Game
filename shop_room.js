class Shop_Room {
    ...
    update(){
        if(this.direction === "left"){
            ...
            this.game.orderCorrecter();
            this.direction = "";
        }else if(this.direction === "SL"||this.direction === "SR"||this.direction === "SU"||this.direction === "SD"){
            this.shopRoom()
        }else{
            this.direction = "";
        }
    ...
    draw(ctx){
        ctx.drawImage(ASSET_MANAGER.getAsset("./res/01_shop_basic.png"),this.locTLX,this.locTLY,769,532);
        ctx.scale(-1,1);
        ctx.drawImage(ASSET_MANAGER.getAsset("./res/01b_shop_basic.png"),this.locTRX,this.locTRY,769,532);
        ctx.restore();
        ctx.scale(-1,-1);
        ctx.drawImage(ASSET_MANAGER.getAsset("./res/01c_shop_basic.png"),this.locBLX,this.locBLY,769,532);
        ctx.restore();
        ctx.scale(-1,1);
        ctx.drawImage(ASSET_MANAGER.getAsset("./res/01d_shop_basic.png"),this.locBRX,this.locBRY,769,532);
        ctx.scale(-1,-1);
        ctx.restore();
